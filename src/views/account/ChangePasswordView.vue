...
function validate(): boolean {
  ...
  if (!currentPassword.value) {
    errors.value.currentPassword = t('validation.currentPasswordRequired')
    ok = false
  }

  const pwd = newPassword.value
  if (pwd.length < MIN_PASSWORD_LEN) {
    errors.value.newPassword = t('validation.newPasswordMinLength', { min: MIN_PASSWORD_LEN })
    ok = false
  } else if (pwd.length > MAX_PASSWORD_LEN) {
    errors.value.newPassword = t('validation.newPasswordMaxLength', { max: MAX_PASSWORD_LEN })
    ok = false
  } else if (pwd === currentPassword.value) {
    errors.value.newPassword = t('validation.newPasswordSameAsCurrent')
    ok = false
  }
...
async function onSubmit() {
  ...
  try {
    ...
    ui.pushToast({ type: 'success', message: t('auth.passwordUpdated') })
  ...
  <div class="mx-auto max-w-lg px-6 py-8 lg:px-10 lg:py-10">
    <div class="mb-6">
      <h1 class="text-xl font-semibold tracking-tight text-slate-900">
        {{ t('auth.changePasswordPageTitle') }}
      </h1>
      <p class="mt-1 text-sm text-slate-500">{{ t('auth.changePasswordPageSubtitle') }}</p>
    </div>
    ...
    >
      <div>
        ...
        <div class="relative">
          ...
          <button
            ...
            :aria-label="currentVisible ? t('common.hidePassword') : t('common.showPassword')"
      ...
      <div>
        <label class="mb-1.5 block text-sm font-medium text-slate-700" for="cp-new">
          {{ t('auth.newPassword') }}
        </label>
        <div class="relative">
          <input
          ...
          <button
            type="button"
            class="absolute right-2 top-1/2 -translate-y-1/2 rounded p-1 text-slate-400 hover:text-slate-600"
            :aria-label="newVisible ? t('common.hidePassword') : t('common.showPassword')"
        ...
        <p v-else class="mt-1 text-xs text-slate-500">{{ t('auth.passwordHint') }}</p>
      </div>
      ...
      <div class="flex gap-2 border-t border-slate-100 pt-4">
        ...
        <button
          ...
          :disabled="submitting"
        >
          {{ submitting ? t('auth.saving') : t('common.save') }}
        </button>
      </div>
    </form>
  </div>
